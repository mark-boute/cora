...
public class TermInterpreter {
  ...
  /**
   * Interpret a list of terms, sharing the same parameter variable map, and return both the list of parameter variables and the list of interpretations.
   * This is useful for interpreting the left and right sides of a dependency pair or rule together, to ensure that they share the same parameter variables.
   * 
   * @param terms
   * @return A pair of the list of parameter variables and the list of interpretations
   */
  ...
  private IntegerExpression _interpret(Term term, Map<Variable, IVar> parameterVariableMap) {
    ...
    List<IntegerExpression> coefficients = _functionInterpretations.get(term.queryRoot()).getCoefficients();
