...
public class RuleTupleInterpretation {
  ...
  protected void addReductionIndicator(IntegerExpression reductionIndicator) {
    _rightSideInterpretation.set(0,
        SmtFactory.createAddition(_rightSideInterpretation.get(0), reductionIndicator).simplify());
  }
  ...
  public void requireWeaklyDecreasing() {
    ...
    for (int i = 0; i < _leftSideInterpretation.size(); i++) {
      ...
      IntegerExpression diff = SmtFactory.createAddition(
          ...
          SmtFactory.createNegation(_rightSideInterpretation.get(i))).simplify();

      Map<IntegerExpression, IntegerExpression> coeffMap = combineTermsOnParameterVariables(diff, parameterVariables);
  ...
  public Pair<List<String>, List<String>> toEvaluatedStrings(Map<IVar, IntegerExpression> valuation) {
    IExpPrinter iExpPrinter = new IExpPrinter();

    List<String> leftInterp = this.getLeftSideInterpretation().stream()
        .map(expr -> iExpPrinter.print(expr.substitute(valuation).simplify()))
        .toList();

    List<String> rightInterp = this.getRightSideInterpretation().stream()
        .map(expr -> iExpPrinter.print(expr.substitute(valuation).simplify()))
        .toList();

    return new Pair<>(leftInterp, rightInterp);
  }
   ...
   * 
   * @return A map from variable terms to their coefficient IntegerExpressions,
   *         and a separate entry for the constant part (with key multIdentity).
   */
  ...
  private static void addCoefficient(Map<IntegerExpression, IntegerExpression> map, IntegerExpression key,
      IntegerExpression valueToAdd) {
    map.compute(key, (k, existingCoeff) -> (existingCoeff == null) ? valueToAdd
...
// yices SMT-ALT
  private void requireCoupledConstraints(Map<IntegerExpression, IntegerExpression> coeffMap,
      List<IVar> parameterVariables) {
    ...
    for (IVar v : parameterVariables) {
      ...
      Optional<Map.Entry<IntegerExpression, IntegerExpression>> squaredEntry = coeffMap.entrySet().stream()
  ...
  private static boolean isSquareOf(IntegerExpression expr, IVar v) {
    if (!(expr instanceof Multiplication mult))
      return false;
    if (mult.numChildren() != 2)
      return false;
